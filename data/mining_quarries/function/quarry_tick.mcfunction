execute as @e[type=minecraft:marker,tag=content_lock.quarry] at @s run execute unless block ~ ~ ~ minecraft:dropper run kill @e[tag=content_lock.quarry.miner,limit=1,sort=nearest,distance=..20]
