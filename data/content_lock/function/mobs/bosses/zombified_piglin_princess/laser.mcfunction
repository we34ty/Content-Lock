...
execute if entity @s[scores={content_lock.attack_timer=200}] at @e[tag=content_lock.im_a_target,limit=1,sort=nearest,type=#minecraft:boss_can_damage] run summon area_effect_cloud ~ ~1 ~ {Duration:21,Tags:["content_lock.zombified_piglin_princess.laser.begin_area"],custom_particle:{type:"block",block_state:"air"}}
