...
execute if entity @s[scores={content_lock.attack_timer=100}] at @e[tag=content_lock.im_a_target,limit=1,sort=nearest,type=#content_lock:boss_can_damage] run summon area_effect_cloud ~ ~ ~ {Duration:21,Tags:[content_lock.zombified_piglin_princess.bandit_bringer.area],custom_particle:{type:"block",block_state:"air"}}
...
execute if entity @s[scores={content_lock.attack_timer=20}] unless score @s content_lock.combo_counter matches 3.. if entity @e[tag=content_lock.im_a_target,type=#content_lock:boss_can_damage] positioned ~-5 ~-5 ~-5 unless entity @e[tag=content_lock.im_a_target,limit=1,sort=nearest,dx=10,dy=10,dz=10] run scoreboard players set @s content_lock.attack_timer 110
