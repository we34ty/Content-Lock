...
execute if entity @s[scores={content_lock.attack_timer=200}] at @e[type=#minecraft:boss_can_damage,distance=..40] run summon area_effect_cloud ~ ~ ~ {Duration:100,Tags:["content_lock.zombified_piglin_princess.rain_fireballs.area"],custom_particle:{type:"block",block_state:"air"}}
