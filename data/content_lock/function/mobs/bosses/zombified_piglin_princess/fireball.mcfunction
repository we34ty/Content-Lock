...
execute if entity @s[scores={content_lock.attack_timer=100}] facing entity @e[tag=content_lock.im_a_target,limit=1,sort=nearest,type=#content_lock:boss_can_damage] eyes run summon area_effect_cloud ~ ~3 ~ {Duration:80,Tags:["content_lock.zombified_piglin_princess.fireball.area"],custom_particle:{type:"block",block_state:"air"}}
