...
execute if entity @s[scores={content_lock.attack_timer=100}] at @e[tag=content_lock.im_a_target,limit=1,sort=nearest,type=#content_lock:boss_can_damage] run summon minecraft:area_effect_cloud ~ ~ ~ {Duration:21,Tags:["content_lock.zombified_piglin_princess.left_right_combo.3.area"]}
