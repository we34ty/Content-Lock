...
execute if entity @s[scores={content_lock.attack_timer=50}] facing entity @e[tag=content_lock.im_a_target,limit=1,sort=nearest,type=#content_lock:boss_can_damage] feet positioned ^ ^ ^20 run summon area_effect_cloud ~ ~ ~ {Duration:41,Tags:["content_lock.zombified_piglin_princess.gunflame.area.point"],custom_particle:{type:"block",block_state:"air"}}
...
#execute if entity @s[scores={content_lock.attack_timer=20}] unless score @s content_lock.combo_counter matches 4.. if entity @e[tag=content_lock.im_a_target,type=#content_lock:boss_can_damage] positioned ~-4 ~-4 ~-4 if entity @e[tag=content_lock.im_a_target,limit=1,sort=nearest,dx=8,dy=8,dz=8] run scoreboard players set @s content_lock.attack_timer 110
