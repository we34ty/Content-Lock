execute if entity @s[tag=content_lock.enchanted_witch] run function animated_java:enchanted_witch/remove/all
execute if entity @s[tag=content_lock.second_officer] run function animated_java:second_officer/remove/all
...
execute if entity @s[tag=content_lock.drop_summon_on_despawn,tag=content_lock.zombified_piglin_princess] run summon item ~ ~ ~ {Item:{id:"pink_dye",components:{custom_data:{"content_lock.zombified_piglin_princess_summon":true},item_model:"minecraft:nether_star",item_name:[{"translate":"content_lock:item.boss_summon.zombified_piglin_princess"}],consumable:{consume_seconds:42342534,animation:"drink"},rarity:"rare",max_stack_size:1}}}
