...
execute if entity @s[scores={content_lock.attack_timer=120}] positioned ~-4 ~-4 ~-4 at @e[dx=8,dz=8,dy=8,type=#content_lock:boss_can_damage] run summon area_effect_cloud ~ ~ ~ {Duration:60,Tags:["content_lock.death_rider.under_punch.area"],custom_particle:{type:"block",block_state:"air"}}
...
execute if entity @s[scores={content_lock.attack_timer=60..99}] as @e[tag=content_lock.death_rider.under_punch.area] at @s facing entity @e[type=#content_lock:boss_can_damage,limit=1,sort=nearest] feet run tp @s ^ ^ ^0.15
