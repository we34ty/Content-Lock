...
execute if entity @s[scores={content_lock.attack_timer=120}] at @e[tag=content_lock.im_a_target,type=#boss_can_damage,limit=1,sort=nearest] run summon area_effect_cloud ~ ~ ~ {Duration:41,Tags:["content_lock.death_rider.spin_teleport.area"],custom_particle:{type:"block",block_state:"air"}}
