...
execute if entity @s[scores={content_lock.attack_timer=131}] facing entity @e[tag=content_lock.im_a_target,type=#boss_can_damage,limit=1,sort=nearest] feet rotated ~ 0 run summon area_effect_cloud ^ ^ ^40 {Duration:32,Tags:[content_lock.death_rider.rush_without_horse.area],custom_particle:{type:"block",block_state:"air"}}
