...
execute if entity @s[scores={content_lock.attack_timer=180}] at @e[type=#minecraft:boss_can_damage,tag=content_lock.im_a_target,limit=1,sort=nearest] run summon area_effect_cloud ~ ~2 ~ {Duration:21,Tags:[content_lock.enchanted_witch.quick_laser.area],Radius:0.0f}
