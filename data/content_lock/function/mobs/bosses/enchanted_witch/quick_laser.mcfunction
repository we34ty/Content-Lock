...
execute if entity @s[scores={content_lock.attack_timer=180}] at @e[type=#minecraft:boss_can_damage,tag=content_lock.im_a_target,limit=1,sort=nearest] run summon area_effect_cloud ~ ~2 ~ {Duration:21,Tags:[content_lock.enchanted_witch.quick_laser.area]}
...
execute if entity @s[scores={content_lock.attack_timer=170}] positioned ~ ~2 ~ facing entity @e[tag=content_lock.enchanted_witch.quick_laser.area,limit=1,sort=nearest] feet run function content_lock:mobs/bosses/enchanted_witch/quick_laser_raycast
execute if entity @s[scores={content_lock.attack_timer=170}] run tag @s remove content_lock.enchanted_witch.quick_laser