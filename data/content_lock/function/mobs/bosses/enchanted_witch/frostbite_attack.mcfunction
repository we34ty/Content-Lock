...
execute if entity @s[scores={content_lock.attack_timer=240}] at @e[type=#content_lock:boss_can_damage,tag=content_lock.im_a_target] run summon area_effect_cloud ~ ~ ~ {Duration:91,Tags:[content_lock.enchanted_witch.frostbite_attack.area],custom_particle:{type:"block",block_state:"air"}}
