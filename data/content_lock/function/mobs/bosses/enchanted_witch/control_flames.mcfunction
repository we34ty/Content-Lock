...
execute if entity @s[scores={content_lock.attack_timer=100}] positioned ~-7 ~-7 ~-7 at @e[type=#content_lock:boss_can_damage,dx=14,dy=14,dz=14] run summon area_effect_cloud ~ ~ ~ {Duration:41,Tags:[content_lock.enchanted_witch.control_flames.area],custom_particle:{type:"block",block_state:"air"}}
execute if entity @s[scores={content_lock.attack_timer=100}] positioned ~-7 ~-7 ~-7 at @e[type=#content_lock:boss_can_damage,dx=14,dy=14,dz=14] run playsound item.firecharge.use hostile @a ~ ~ ~ 1 1
