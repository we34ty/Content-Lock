...
execute unless score @s content_lock.temp1 matches 1 at @s if dimension minecraft:overworld if entity @s[y=-30,dy=30] store result score @s content_lock.temp1 run function content_lock:check_random_chance {chance:50}
execute unless score @s content_lock.temp1 matches 1 at @s if dimension minecraft:overworld if entity @s[y=-50,dy=20] store result score @s content_lock.temp1 run function content_lock:check_random_chance {chance:25}
execute unless score @s content_lock.temp1 matches 1 at @s if dimension minecraft:overworld if entity @s[y=-64,dy=14] store result score @s content_lock.temp1 run function content_lock:check_random_chance {chance:12}
