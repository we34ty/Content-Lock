execute if score L content_lock.options.enemy.scaling matches 1 at @a as @e[type=#can_scale,tag=!content_lock.progression_scaled,tag=!content_lock.progression_not_scaled,distance=..40] at @s run function content_lock:mobs/passives/progression_scaling
...
execute if score L content_lock.timer1 matches 12 if entity @e[type=#invisibility,team=!content_lock.invisibility] as @e[type=#invisibility,team=!content_lock.invisibility] run team join content_lock.invisibility
