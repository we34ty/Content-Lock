execute unless block ~ ~ ~ #replaceable as @p unless entity @s[gamemode=creative] run give @s zombie_spawn_egg[item_name={translate:"content_lock:item.duplicator",italic:false},entity_data={id:"marker",Tags:["content_lock.duplicator"]},item_model="content_lock:duplicator"] 1
