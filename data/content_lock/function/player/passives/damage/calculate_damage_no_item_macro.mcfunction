$execute unless data storage content_lock:saved_stats damage.$(type) unless data storage content_lock:saved_stats damage_modifiers.$(type)[0] run return 0
$execute store result score @s content_lock.temp2 run data get storage content_lock:saved_stats $(type)_damage 1000
