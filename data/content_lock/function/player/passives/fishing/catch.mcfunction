##grants some fishing loot and removes the current fish instance
...
execute at @n[tag=content_lock.fishing.fish.current] as @p[tag=content_lock.fishing.hooked_tag] run loot spawn ~ ~ ~ fish gameplay/fishing ~ ~-0.2 ~
...
advancement grant @s only husbandry/fishy_business
execute at @n[tag=content_lock.fishing.fish.current] if predicate content_lock:underground_pond run advancement grant @s only content_lock:husbandry/fish_in_underground_pond
