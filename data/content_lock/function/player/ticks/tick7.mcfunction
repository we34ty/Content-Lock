execute if score @r[scores={content_lock.time_since_death=21..}] content_lock.time_since_rest matches 1..20 if predicate content_lock:is_day run function content_lock:player/passives/sleep/night_skipped_check
...
execute if score @r[scores={content_lock.time_since_death=21..}] content_lock.time_since_rest matches 0 as @a[scores={content_lock.time_since_rest=0,content_lock.time_since_death=21..}] at @s run function content_lock:player/passives/sleep/sleeping
