...
data merge storage content_lock:weapon_stats {Id:0,data:{UUID:[I;0,0,0,0],damage:0,distance:0,pitch:0,yaw:0}}
...
execute store result storage content_lock:weapon_stats data.yaw int 1 run data get entity @n[tag=content_lock.weapons.attacking] Rotation[0]
execute store result storage content_lock:weapon_stats data.pitch int 1 run data get entity @n[tag=content_lock.weapons.attacking] Rotation[1]
