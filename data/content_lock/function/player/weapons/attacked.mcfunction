...
data merge storage content_lock:saved_stats {angle:0,distance:0}
execute store result storage content_lock:saved_stats angle int 1 run data get entity @n[type=minecraft:armor_stand,tag=content_lock.weapons.angle,distance=..5] Rotation[1] -1000
kill @n[type=minecraft:armor_stand,tag=content_lock.weapons.angle,distance=..5]
execute store result storage content_lock:saved_stats distance float 0.0008 run function content_lock:math/tan with storage content_lock:saved_stats

execute if items entity @s weapon.mainhand #minecraft:swords run function content_lock:player/weapons/types/sword with storage content_lock:saved_stats
