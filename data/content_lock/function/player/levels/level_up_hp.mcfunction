execute unless score @s content_lock.player_unused_skill_points matches 1.. run scoreboard players enable @s content_lock.LevelUp.hp
...
execute if score @s content_lock.player_unused_skill_points matches 1.. run tellraw @s [{"translate":"content_lock:levels.hp.increase","color":"yellow"},{"score":{"name":"@s","objective":"content_lock.hp_from_level"},"color": "yellow"}]
