...
$execute if score @s content_lock.undead_book_charge matches 1..$(attack_speed) positioned ~-$(half_range) ~-$(half_range) ~-$(half_range) at @e[type=!#entities,dx=$(range),dy=$(range),dz=$(range),nbt=!{UUID:$(UUID)}] run particle block_crumble{block_state:dirt} ~ ~ ~ 0.5 0.5 0.5 1 10 normal
$execute if score @s content_lock.undead_book_charge matches 1..$(attack_speed) positioned ~-$(half_range) ~-$(half_range) ~-$(half_range) as @e[type=!#entities,dx=$(range),dy=$(range),dz=$(range),nbt=!{UUID:$(UUID)}] run effect give @s slowness 1 0 false
...
$execute if score @s content_lock.undead_book_charge matches $(attack_speed) positioned ~-$(half_range) ~-$(half_range) ~-$(half_range) as @e[type=!#entities,dx=$(range),dy=$(range),dz=$(range),nbt=!{UUID:$(UUID)}] run damage @s $(damage) fly_into_wall by @p[nbt={UUID:$(UUID)}]
